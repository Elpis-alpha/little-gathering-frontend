...
import { setTestedData, setUserData } from '../../store/actions/userAction'
...
const InitialLoader = ({ status }) => {
  ...
  useEffect(async () => {
    ...
    dispatch(setTestedData(true))

    if (userData) { if (!userData.error) { dispatch(setUserData(userData)) } }
  ...
  return (
    ...
    <InitLoaderStyle className={status}>
      ...
      <div>
        ...
        <h1>Welcome to Little Gathering</h1>
