...
import { postComments, getUserPicture, likePost, dislikePost } from "../../api"
...
import { setCommentShow } from "../../store/actions/commentAction"
...
const Post = ({ post }) => {
  ...
  const { show: showCommentForm } = useSelector(store => store.commentForm)
  ...
  const [readMore, setreadMore] = useState(post.readMore)
  ...
  useEffect(async () => {
    ...
    const commentResponse = await fetch(postComments(0, 3, post._id))
  ...
  useEffect(async () => {
    ...
    userImage.src = getUserPicture(post.owner)

    userImage.onload = () => {

      try {

        userImage.alt = post.name
  ...
  const saySomethingHandler = () => {
    ...
    if (post._id === showCommentForm) {
      ...
      dispatch(setCommentShow(post._id))
  ...
  return (
    ...
    <PostStyle>
      ...
      <div className="post-holder">
        ...
        <div className="post-content">{postText} {readMore && <button onClick={readMoreHandler}>Read more</button>}</div>
        ...
        <div className="post-modifications">
          ...
          <div className="left-side">
            ...
            {available && (data.email === post.email && <>
        ...
        <div className="comments-holder"><CommentHolder comments={comments} _id={post._id} /></div>
